(function (factory) {
  ...
  window.onload = function() {
    ...
    function display(ctx, apparatus, options) {
      ...
      let justify_x = new justeer(canvas.width, nx, apparatus.xdim * (options.cell_size + options.cell_pad));
      ...
      let place_y = justify_y.placement_given_spacing_between_elements((Math.sqrt(3) * padding) / 2);
      ...
      for (let i = 0; i < ny; i++) {
        for (let j = 0; j < nx - (i % 2); j++) {
          ...
          const offset = (apparatus.xdim * (options.cell_size + options.cell_pad) + padding) / 2;
    ...
    function display_apparatus2(ctx, rects, options) {
      ...
      const roughRects = rects.map(rect => {
        const points = getRectPoints(rect.x1 * (cell_size + cell_pad), rect.y1 * (cell_size + cell_pad), rect.w * (cell_size + cell_pad) - cell_pad, rect.h * (cell_size + cell_pad) - cell_pad);
        return { ...rect, points };
      });
      ...
      if (display_stroke) {
        roughRects.forEach(rect => {
          const points = getRectPoints(rect.x1 * (cell_size + cell_pad) + 4, rect.y1 * (cell_size + cell_pad) + 4, rect.w * (cell_size + cell_pad) - cell_pad - 8, rect.h * (cell_size + cell_pad) - cell_pad - 8);
          const hatchPoints = getHatchPoints(rect.x1 * (cell_size + cell_pad), rect.y1 * (cell_size + cell_pad), rect.w * (cell_size + cell_pad) - cell_pad, rect.h * (cell_size + cell_pad) - cell_pad);
    ...
    function drawPoints(ctx, points, col, strokeCol) {
      ...
      for (let i = 1; i < points.length; i++) {
        let p1 = points[i - 1];
        let p2 = points[i];
        ctx.bezierCurveTo(p1.cp2 ? p1.cp2[0] : p1.a[0], p1.cp2 ? p1.cp2[1] : p1.a[1], p2.cp1 ? p2.cp1[0] : p2.a[0], p2.cp1 ? p2.cp1[1] : p2.a[1], p2.a[0], p2.a[1]);
      }
      if (col != null) {
        ctx.fillStyle = col;
        ctx.fill();
